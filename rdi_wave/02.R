...
if (buf[i0] == 0x7f && buf[i0 + 1] == 0x79) {
    ...
    oceDebug(debug, "    ", vectorShow(checksumOffset, postscript=" i0+2:3 (maybe skip forward by this?"))
    numberOfDataTypes <- as.integer(buf[i0 + 4])
    oceDebug(debug, "    ", vectorShow(numberOfDataTypes))
    offset <- readBin(buf[seq(i0+5, length.out=2L*numberOfDataTypes)], "integer", n=numberOfDataTypes, size=2, endian="little")
    ...
    if (buf[i0 + 8] == 0x03 && buf[i0 + 9] == 0x01) {
        ...
        configuration <- readBin(buf[i0 + 12:13], "integer", size=2)
        oceDebug(debug, "    ", vectorShow(configuration, postscript="bit field (not decoded here)"))
...
oceDebug(debug, vectorShow(i0 + 3:4 + checksumOffset))
oceDebug(debug, vectorShow(buf[i0 + 3:4 + checksumOffset], postscript=" -- is this a code?"))
